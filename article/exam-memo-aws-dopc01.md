---
title: "【対策メモ】AWS認定DevOpsエンジニア-プロフェッショナル試験"
...
本記事は、AWS-DOP試験の対策をしている上で覚えておくべきだと感じた内容を残しておくものです。
主に問題演習中に間違えたものを記載しています。
...
- appspec.yml
  - hooks
    ...
    - ValidateService
      - サービスの検証が終わった後
  - デプロイログに報告されないデプロイの失敗
    - ALBターゲットグループに指定されたヘルスチェックが正しく設定されていない
...
- ハイブリッド環境でのSSM
  - ハイブリッドインスタンスのIDは「mi~」になり、EC2インスタンスと区別される
...
- S3 MFA delete
  - s3のバージョニング機能のオプション
  - オブジェクトのバージョンを削除する際にMFAの入力が必要になる(セキュリティ強化)
